...
def is_paired_fastq(file_path):
    ...
    if str(file_path) == "/snode_data/meishi/PRJEB15371_123/batch_kneaddata_results/ERR1620377_kneaddata_out/ERR1620377_1_kneaddata_paired_2.fastq.gz":
        print(f"re {bool(pattern.search(str(file_path)))}")
...
def get_parse_sample_table_by_sample_table(sample_table, failed_sample):
    ...
    with open(sample_table, "r") as f:
        for line in f:
            ...
            for file_path in reads_path.rglob("*"):
                if str(file_path) == "/snode_data/meishi/PRJEB15371_123/batch_kneaddata_results/ERR1620377_kneaddata_out/ERR1620377_1_kneaddata_paired_2.fastq.gz":
                    print("found")
                if not is_paired_fastq(file_path): 
                    continue
                print(file_path)
    ...
    if len(failed_sample_list) > 0:
        for sample in failed_sample_list:
            try:
                if all_sample2infos.get(sample):
                    del all_sample2infos[sample]
            except KeyError as e:
                print(f"KeyError: {e}") 
    print(all_sample2infos)
...
sample_table = "/home/work/wenhai/gut/data/samples_test.tsv"
failed_sample = "/home/work/wenhai/gut/data/failed_sample.txt"
parse_sample_table = "None"
fastq_info = "/home/work/wenhai/gut/data/fastq_info"
new_parse_sample_table = "/home/work/wenhai/gut/data/parse_samples_table_test.tsv"
